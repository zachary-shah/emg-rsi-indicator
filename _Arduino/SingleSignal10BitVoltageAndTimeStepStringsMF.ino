...
const uint8_t target_adc_frequency_in_kHz = 1;
const uint8_t adc_pins[] = {0};
const uint8_t adc_pins_count = sizeof(adc_pins);
volatile uint8_t adc_pin_counter = 0;
...
{ 
  ...
  serial_string += String (adc_now) + "," + String (current_time - last_time) + "\n";
